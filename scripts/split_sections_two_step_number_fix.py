...
APPENDIX_ALIASES_PREFIX = ("appendix",)  # matched with startswith
...
def classify_section(text: str) -> str | None:
    ...
    if normalized in INTRODUCTION_ALIASES:
        return "introduction"
    if any(normalized.startswith(p) for p in APPENDIX_ALIASES_PREFIX):
        return "appendix"
...
def parse_paper(blocks: list[dict]) -> dict:
    ...
    for block in blocks:
        ...
        if title_info:
            ...
            # Experiment heading (may be numbered "5. Experiment 1")
            ...
            if section_type == "article_info":
                result["frontmatter"]["article_info"].append(block)
                stage = "article_info"
                continue
            ...
            # Explicit Introduction heading (e.g. "1. Introduction")
            ...
            if stage == "extended_intro":
                # Extended intro subsection (like "2. The hit rate portion...")
                result["frontmatter"]["introduction"].append(block)
                continue
            if in_backmatter and current_section:
                result["backmatter"][current_section].append(block)
                continue
            if current_experiment and current_section:
        ...
        # Article info block (Elsevier "Article history: Received..." etc.)
        if stage == "article_info":
            if btype == "paragraph":
        ...
        # Abstract body (after "a b s t r a c t" heading)
        if stage == "abstract_body":
            result["frontmatter"]["abstract"].append(block)
            continue

        # Extended intro body (after "1. Introduction" or its subsection headings)
        if stage == "extended_intro":
            result["frontmatter"]["introduction"].append(block)
            continue

        # Traditional frontmatter routing (papers without Elsevier markers)
        if btype == "paragraph":
            text = get_paragraph_text(block)
