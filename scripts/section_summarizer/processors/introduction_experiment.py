"""
Experiment-level introduction processor.

Injects paper-level introduction context into the prompt so the LLM can
distinguish restatements from genuine new theoretical content.
"""
...
from ..schemas.introduction_experiment import IntroductionExperimentSummary
...
NO_PRIOR_CONTEXT_MESSAGE = (
    ...
    "You must judge novelty conservatively: only include statements in "
    "`theoretical_refinements` or `new_alternative_theories` when the "
    "experiment intro clearly presents them as new, refined, or "
    "alternative accounts.)"
)


class IntroductionExperimentProcessor(BaseProcessor):
    task_name = "introduction_experiment"
    schema_cls = IntroductionExperimentSummary
    prompt_filename = "introduction_experiment.txt"
    template_filename = "introduction_experiment.md.j2"
    output_stem = "introduction"   # writes to exp_N/introduction.{json,md}

    # -- Override prompt building to inject paper-level context ------------
    ...
    def _load_prior_context(cfg: AppConfig, paper_name: str) -> str:
        """
        Load the paper-level introduction JSON and format its theoretical
        content as prior context. Returns a fallback message if the file
        is missing.
        """
...
if __name__ == "__main__":
    """
    Usage:
      uv run python -m scripts.section_summarizer.processors.introduction_experiment <paper> [--force] [--rerender]
    """
    import sys
    ...
    if not args:
        print(
            ...
            "introduction_experiment <paper_name> [--force] [--rerender]"
        )
        sys.exit(1)
    ...
    cfg = load_config()
    client = LLMClient()
    processor = IntroductionExperimentProcessor(cfg, client)

    processed = 0
    skipped = 0
    for section in discover_sections(cfg, paper_name):
        if section.task_name != "introduction_experiment":
            continue
        if rerender_only:
